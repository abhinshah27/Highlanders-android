...
import rs.highlande.highlanders_app.utility.FileFromUriListener;
import rs.highlande.highlanders_app.utility.FileFromUriTask;
import rs.highlande.highlanders_app.utility.GlideApp;
import rs.highlande.highlanders_app.utility.LogUtils;
import rs.highlande.highlanders_app.utility.ProgressRequestBody;
...
//                      .apply(RequestOptions.onlyRetrieveFromCache(true)
	...
	public static void loadProfilePictureWithPlaceholder(Context context, String url, ImageView target) {
		loadPictureWithGlide(context, url, null, R.drawable.ic_profile_placeholder/* -1*/,
	...
	public static void checkPermissionForCustomGallery(HLActivity activity, Fragment fragment) {
		...
		if (Utils.hasMarshmallow()) {
			...
			if (!hasRead) {
				...
				LoaderManager.getInstance(activity).restartLoader(0, null, callbacks);
			}
		}
		else LoaderManager.getInstance(activity).restartLoader(0, null, callbacks);
...
//		intent.setType(MIME_PNG);           // image PNG
	...
	private static final String MEDIA_UPLOAD_VALUE = "hfu74365ksdg0wrnhq52FUW#$(kjdhgf";
	...
	public static class UploadService {
		 ...
		 * @param context  the application's/activity's {@link Context} instance.
		 * @param file     the file to be uploaded.
		 * @param userId   the {@link rs.highlande.highlanders_app.models.HLUser#_id} of the user uploading the picture.
		 * @param listener the callback receiver when uploading is concluded.
		 * @return True if the operation is successful.
		 * @throws IOException if the operation is not successful.
		 * @throws IllegalArgumentException if some of or all the provided pieces are invalid.
