...
import rs.highlande.highlanders_app.services.FCMNotNeededException;
            ...
            if (/*wantsGroup = */(Utils.hasNougat() && parts.length == 2 && Utils.areStringsValid(parts[0], parts[1]))) {
                if (Utils.hasPie())
                    ...
                    style = new NotificationCompat.MessagingStyle("Me")
                            .addMessage(new NotificationCompat.MessagingStyle.Message(parts[1].substring(1), when, parts[0]));
            }
